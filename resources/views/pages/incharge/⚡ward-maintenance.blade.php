...
new #[Title('Ward Maintenance')] class extends Component
{
    #[Url]
    ...
    {
        return WardMaintenanceEntry::with(['answers', 'faults', 'user'])
            ->whereDate('checklist_date', $this->selectedDate)
            ->get()
            ->keyBy(fn (WardMaintenanceEntry $entry) => $entry->shift->value);
    }

    /**
     * Find the submitted entry for a shift on the selected date.
     */
    public function entryFor(WardMaintenanceShift $shift): ?WardMaintenanceEntry
    {
        ...
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <flux:text class="text-zinc-500">
                    {{ __('Gyne Ward & Private Rooms — fill once per shift.') }}
                </flux:text>
            </div>
            <flux:field>
                ...
                <flux:card wire:key="shift-{{ $shift->value }}-{{ $selectedDate }}">
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="flex justify-end">
                            <flux:button
                                variant="primary"
                                :href="route('incharge.ward-maintenance.form', ['shift' => $shift->value, 'date' => $selectedDate])"
                                wire:navigate
                            >
