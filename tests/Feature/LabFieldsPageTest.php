...
test('a display name can be set when creating and editing a field', function () {
    $labTechnician = User::factory()->labTechnician()->create();
    $labTest = LabTest::factory()->create();

    $component = Livewire::actingAs($labTechnician)
        ->test('pages::lab.test-fields', ['labTest' => $labTest])
        ->set('newFieldName', 'WBC')
        ->set('newFieldDisplayName', ' Total Leucocyte Count ')
        ->call('createAndAttachField')
        ->assertHasNoErrors()
        ->assertSee('On report: Total Leucocyte Count');

    $field = LabField::where('name', 'WBC')->firstOrFail();

    expect($field->display_name)->toBe('Total Leucocyte Count');

    $component->call('openEditFieldModal', $field->id)
        ->assertSet('editFieldDisplayName', 'Total Leucocyte Count')
        ->set('editFieldDisplayName', '')
        ->call('updateField')
        ->assertHasNoErrors();

    expect($field->fresh()->display_name)->toBeNull();
});

test('the all fields page shows and searches display names', function () {
    $labTechnician = User::factory()->labTechnician()->create();
    LabField::factory()->create(['name' => 'WBC', 'display_name' => 'Total Leucocyte Count']);
    LabField::factory()->create(['name' => 'Calcium']);

    Livewire::actingAs($labTechnician)
        ->test('pages::lab.fields')
        ->set('search', 'leucocyte')
        ->assertSee('WBC')
        ->assertSee('On report: Total Leucocyte Count')
        ->assertDontSee('Calcium');
});
