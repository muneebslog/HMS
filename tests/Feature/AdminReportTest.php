...
test('receptionist reply to an admin-started thread pushes priority 5', function () {
    $receptionist = User::factory()->receptionist()->create();
    $admin = User::factory()->admin()->create();

    $report = AdminReport::factory()->create([
        'created_by' => $admin->id,
        'subject' => 'Please check the queue TV',
    ]);

    AdminReportMessage::factory()->create([
        'admin_report_id' => $report->id,
        'user_id' => $admin->id,
        'body' => 'Is the TV frozen again?',
    ]);
    ...
    Livewire::actingAs($receptionist)
        ->test('report-to-admin')
        ->call('selectReport', $report->id)
        ->set('replyBody', 'Yes, restarting it now.')
        ->call('reply')
        ->assertHasNoErrors();
...
test('admin and receptionist can see shared report thread messages', function () {
    $receptionist = User::factory()->receptionist()->create();
    $admin = User::factory()->admin()->create();
